...
const Openings = () => {
  ...
  return (
    <section className="bg-white py-16 px-4 sm:px-8 md:px-12 max-w-7xl mx-auto font-sans">
      ...
      {/* Job Listings */}
      {showJobs && (
        <div  ref={jobListRef} className="pt-28 max-w-5xl mx-auto" id="job-listings">
          <h3 className="text-3xl font-semibold mb-8 text-gray-800 text-center">Available Job Openings</h3>

          {jobs.length === 0 ? (
            <p className="text-gray-500 text-center">No jobs available at the moment.</p>
          ) : (
            <ul className="grid sm:grid-cols-2 lg:grid-cols-2 gap-8">
              {jobs.map((job) => (
                <li
                  key={job._id}
